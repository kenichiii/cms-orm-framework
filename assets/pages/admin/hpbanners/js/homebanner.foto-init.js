...
if (("addEventListener" in window) && ("FileReader" in window) && ("FormData" in window)) {
 ...
 $("#homebanner-foto-file-output").html("Upload obrázku není Vaším prohlížečem podporován");
}

$("#homebanner-foto-file-upload").hide();
$("#homebanner-foto-button-upload").unbind('click').click(function(){
    $("#homebanner-foto-file-upload").trigger('click');
    return false;
});    
    
var 
    filesUpload = document.getElementById("homebanner-foto-file-upload"),
    fileList = document.getElementById("homebanner-foto-file-output")
  ;
...
function uploadFile (file) {
    ...
    progressBarContainer.className = "homebanner-foto-progress-bar-container";
    progressBar.className = "homebanner-foto-progress-bar";
    ...
    formData.append('photo', file, file.name);
        ...
        xhr.open("post", $('#homebanner-foto-upload-form').attr('action'), true);
...
{
    //delete button
    $("#homebanner-foto-delete").unbind('click').click(function(){
           var that = this; 
...
function loadFoto()
{
    $.get($("#homebanner-foto-url").val(),{},function(img){    
        $(".homebanner-foto-img-holder").html(img);
        activateFotoListeners();
    });
}