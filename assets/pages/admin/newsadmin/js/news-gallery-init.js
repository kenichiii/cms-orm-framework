...
{
    $("#news_gallery .gallery-admin-holder").html("Nahrávám...");
    $.get($("#news-gallery-list-url").val(),{id:$("#news-gallery-ownerid").val()},function(html){
        $("#news_gallery .gallery-admin-holder").html(html);
        initGallery();
    });
...
if (("addEventListener" in window) && ("FileReader" in window) && ("FormData" in window)) {
 ...
 $("#news-gallery-file-list").html("Upload obrázků do galerie není Vaším prohlížečem podporován");
    //document.getElementById("browsers").style.display = "block";
}

$("#news-gallery-files-upload").hide();
$("#news-gallery-button-upload").unbind('click').click(function(){
    $("#news-gallery-files-upload").trigger('click');
    return false;
});

//rewrite https://github.com/Integralist/XHR2-Multiple-File-Upload--with-PHP-/blob/master/upload.js
var filesUpload = document.getElementById("news-gallery-files-upload"),
fileList = document.getElementById("news-gallery-file-list");
...
xhr.open("post", $('#news_gallery_form').attr('action'), true);
...
function initGallery() {
    ...
    $("#news_gallery .gallery-img:odd").css('background-color','lightgray')

		$("#news_gallery .gallery-admin-list").sortable({
                    ...
                    update : function (event, ui) {
                                ...
                                $.get($("#news-gallery-sort-url").val(),
                                    {
                                        photo: id.replace('photo_',''),
                                        neib: neib.replace('photo_',''),
                                        ownerid:$("#news-gallery-ownerid").val()        
                                    }, function(result) {
                                        if( result == 'error' )
        ...
        $('#news_gallery .gallery-edit').unbind('click').click(function(){
            ...
            dialog.load(
                ...
                function (responseText, textStatus, XMLHttpRequest) {
                    dialog.dialog({
                        ...
                        $("#galerieformedit input[name='ownerid']").val($("#news-gallery-ownerid").val());
        ...
        $('#news_gallery .gallery-delete').unbind('click').click(function(){
                     var that = this;
                     showConfirm('Opravdu smazat obrázek?',function(){
                            $.get( that.href, {ownerid:$("#news-gallery-ownerid").val()},
                            function(res)
                            {
                                if( res == 'error' )
                                    showAlert('error',{mtype:'err'});
