...
if (("addEventListener" in window) && ("FileReader" in window) && ("FormData" in window)) {
 ...
 $("#reference-foto-file-output").html("Upload obrázku není Vaším prohlížečem podporován");
}

$("#reference-foto-file-upload").hide();
$("#reference-foto-button-upload").unbind('click').click(function(){
    $("#reference-foto-file-upload").trigger('click');
    return false;
});    

var 
    filesUpload = document.getElementById("reference-foto-file-upload"),
    fileList = document.getElementById("reference-foto-file-output")
  ;
...
function uploadFile (file) {
    ...
    progressBarContainer.className = "reference-foto-progress-bar-container";
    progressBar.className = "reference-foto-progress-bar";
    ...
    formData.append('photo', file, file.name);
        ...
        xhr.open("post", $('#reference-foto-upload-form').attr('action'), true);
...
{
    //delete button
    $("#reference-foto-delete").unbind('click').click(function(){
           var that = this; 
...
function loadFoto()
{
    $.get($("#reference-foto-url").val(),{},function(img){    
        $(".reference-foto-img-holder").html(img);
        activateFotoListeners();
    });
}