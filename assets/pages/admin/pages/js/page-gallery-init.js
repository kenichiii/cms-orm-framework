...
{
    $("#page_gallery .gallery-admin-holder").html("Nahrávám...");
    $.get($("#page-gallery-list-url").val(),{id:$("#page-gallery-ownerid").val()},function(html){
        $("#page_gallery .gallery-admin-holder").html(html);
        initGallery();
    });
...
if (("addEventListener" in window) && ("FileReader" in window) && ("FormData" in window)) {
 ...
 $("#page-gallery-file-list").html("Upload obrázků do galerie není Vaším prohlížečem podporován");
    //document.getElementById("browsers").style.display = "block";
}

$("#page-gallery-files-upload").hide();
$("#page-gallery-button-upload").unbind('click').click(function(){
    $("#page-gallery-files-upload").trigger('click');
    return false;
});

//rewrite https://github.com/Integralist/XHR2-Multiple-File-Upload--with-PHP-/blob/master/upload.js
var filesUpload = document.getElementById("page-gallery-files-upload"),
fileList = document.getElementById("page-gallery-file-list");
...
xhr.open("post", $('#page_gallery_form').attr('action'), true);
...
function initGallery() {
    ...
    $("#page_gallery .gallery-img:odd").css('background-color','lightgray')

		$("#page_gallery .gallery-admin-list").sortable({
                    ...
                    update : function (event, ui) {
                                ...
                                $.get($("#page-gallery-sort-url").val(),
                                    {
                                        photo: id.replace('photo_',''),
                                        neib: neib.replace('photo_',''),
                                        ownerid:$("#page-gallery-ownerid").val()        
                                    }, function(result) {
                                        if( result == 'error' )
        ...
        $('#page_gallery .gallery-edit').unbind('click').click(function(){
            ...
            dialog.load(
                ...
                function (responseText, textStatus, XMLHttpRequest) {
                    dialog.dialog({
                        ...
                        $("#galerieformedit input[name='ownerid']").val($("#page-gallery-ownerid").val());
        ...
        $('#page_gallery .gallery-delete').unbind('click').click(function(){
                     var that = this;
                     showConfirm('Opravdu smazat obrázek?',function(){
                            $.get( that.href, {ownerid:$("#page-gallery-ownerid").val()},
                            function(res)
                            {
                                if( res == 'error' )
                                    showAlert('error',{mtype:'err'});
